...
* EKS Managed Capabilities - AWS-managed capabilities that run in AWS-owned infrastructure (not on your cluster nodes). These are enabled via `cluster_config.capabilities` in your tfvars file:
  * **ArgoCD** (`gitops = true`) - GitOps continuous deployment, managed by AWS. See [EKS ArgoCD docs](https://docs.aws.amazon.com/eks/latest/userguide/argocd.html)
  * **KRO** (`kro = true`) - Kube Resource Orchestrator for composing custom Kubernetes APIs. See [EKS KRO docs](https://docs.aws.amazon.com/eks/latest/userguide/kro.html)
  * **ACK** (`ack = true`) - AWS Controllers for Kubernetes, manage AWS resources via K8s APIs. See [EKS ACK docs](https://docs.aws.amazon.com/eks/latest/userguide/ack.html)

## Prerequisites

* **AWS Identity Center (required for ArgoCD capability)**: The ArgoCD EKS Capability requires [AWS Identity Center](https://docs.aws.amazon.com/singlesignon/latest/userguide/what-is.html) (formerly AWS SSO) for authentication. You must have an Identity Center instance configured before enabling ArgoCD. Set the instance ARN and region in `cluster_config.capabilities_config` in your tfvars file. If Identity Center is not configured, set `gitops = false` — KRO and ACK do not require it.
* **VPC quota**: Ensure your account has available VPC quota in the target region (default limit is 5 per region). Check with `aws ec2 describe-vpcs --query 'length(Vpcs)'`.
