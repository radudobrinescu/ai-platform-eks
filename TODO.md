...
- [ ] **Terraform-managed MinIO password for Langfuse** — Generate the MinIO password in Terraform (`random_password`), add it to the `langfuse-secrets` Kubernetes Secret, and configure the Langfuse Helm chart to use it natively. Currently the MinIO password is auto-generated by the Helm chart and injected into Langfuse via a hacky `additionalEnv` workaround. The proper fix ensures all secrets flow through Terraform consistently.
