...
- [ ] **Terraform-managed MinIO password for Langfuse** — If blob storage is re-enabled in the future (for large-scale production), generate the MinIO password in Terraform (`random_password`), add it to the `langfuse-secrets` Kubernetes Secret, and configure the Langfuse Helm chart to use it natively. Currently blob storage is disabled (`LANGFUSE_ENABLE_BLOB_STORAGE=false`) and events are stored directly in ClickHouse.
